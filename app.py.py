...
Quais fatores estão mais associados à ocorrência de acidentes em rodovias federais brasileiras e quais padrões podem auxiliar na prevenção desses eventos?
""")
...
def gerar_dados():
    ...
    datas = inicio + pd.to_timedelta(
        np.random.randint(0, (fim - inicio).days + 1, size=n),
        unit="D"
    )

    hora_base = np.random.randint(0, 24, n)
    ...
    df = pd.DataFrame({
        ...
        "mortos": np.random.poisson(0.2, n),
        "feridos_graves": np.random.poisson(0.5, n),
        "feridos_leves": np.random.poisson(1.5, n),
...
def limpar(df):
    ...
    if "horario" in df.columns:
        ...
        df["hora_int"] = pd.to_numeric(
            df["hora_int"], errors="coerce").fillna(0).astype(int)
    else:
        df["hora_int"] = 0
    ...
    vitimas = [c for c in ["mortos", "feridos_graves",
        "feridos_leves"] if c in df.columns]
...
faixas = ["Madrugada (00-06)", "Manhã (06-12)",
...
for b, v in zip(barras, top_tipos.values):
    ax1.text(v + 10, b.get_y() + b.get_height() / \
             2, str(v), va="center", fontsize=8)
...
ax2.plot(por_hora.index, por_hora.values, color="#2E86C1",
         linewidth=2, marker="o", markersize=4)
ax2.fill_between(por_hora.index, por_hora.values, alpha=0.15, color="#2E86C1")
ax2.axvline(x=por_hora.idxmax(), color="#C0392B", linestyle="--",
            linewidth=1.5, label=f"Pico: {por_hora.idxmax()}h")
...
cores = ["#C0392B" if i == 0 else "#E67E22" if i ==
    1 else "#7F8C8D" for i in range(len(top_causas))]
...
st.pyplot(fig3)
st.markdown(
    "> Falta de atenção e velocidade incompatível somam quase metade dos acidentes registrados.")
...
# Gráfico 5
st.markdown("---")
st.subheader("5. Estados com Maior Índice Médio de Gravidade")

gravidade_uf = (
    df.groupby("uf")["indice_gravidade"]
    .mean()
...
ax5.bar(gravidade_uf.index, gravidade_uf.values)
ax5.set_title("Top 10 Estados por Gravidade Média dos Acidentes")
ax5.set_xlabel("UF")
ax5.set_ylabel("Índice Médio de Gravidade")

plt.tight_layout()
st.pyplot(fig5)

uf_mais_grave = gravidade_uf.index[0]

st.markdown(
    f"> A UF com maior índice médio de gravidade foi **{uf_mais_grave}**, indicando maior severidade média dos acidentes registrados."
)
...
st.dataframe(
    df[cols].head(100).reset_index(drop=True),
